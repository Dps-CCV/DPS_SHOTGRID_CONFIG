...
class MayaParticlesGeometryPublishPlugin(HookBaseClass):
    """
    ...
    def settings(self):
        ...
        base_settings = super(MayaParticlesGeometryPublishPlugin, self).settings or {}
    ...
    def accept(self, settings, item):
        ...
        if publisher.context.step['name'] in ['TXT', 'SHA', 'LAY']:
    ...
    def validate(self, settings, item):
        ...
        return super(MayaParticlesGeometryPublishPlugin, self).validate(settings, item)
    ...
    def publish(self, settings, item):
        ...
        # ...and execute it:
        try:
            self.parent.log_debug("Executing command Ass Export:")
            cmds.file(publish_path.replace("\\", "/"), exportSelected= True, force=True, type="ASS Export", options="-shadowLinks 1;-mask 6399;-lightLinks 1;-fullPath" )
        except Exception as e:
            self.logger.error("Failed to export Ass: %s" % e)
            return
        item.properties["publish_type"] = "Ass Cache"

        # Now that the path has been generated, hand it off to the
        super(MayaParticlesGeometryPublishPlugin, self).publish(settings, item)
