...
class NukeActions(HookBaseClass):
    ...
    def generate_actions(self, sg_publish_data, actions, ui_area):
        ...
        if "liveGroup_node" in actions:
            action_instances.append(
                {
                    "name": "liveGroup_node",
                    "params": None,
                    "caption": "Import Script into a liveGroup node",
                    "description": "This will import the script into a liveGroup node.",
                }
            )
            ...
            action_instances.append(
                {
                    ...
                    "description": "This will import a publish as a camera node.",
                }
            )
        if "audioread_node" in actions:
            action_instances.append(
                {
                    "name": "audioread_node",
                    "params": None,
                    "caption": "Import Audio",
                    "description": "This will import a wav file as an audioread node.",
                }
            )
    ...
    def execute_action(self, name, params, sg_publish_data):
        ...
        if name == "liveGroup_node":
            print ("KJKJKJK")
            self._script_to_liveGroup(path, sg_publish_data)
        ...
        if name == "audioread_node":
            self._create_audioread_node(path, sg_publish_data)
    ...
    def _script_to_liveGroup(self, path, sg_publish_data):
        """
        Import contents of the given file into the scene.
        ...
        print ("HHHH")
        print (path)
        liveGroup = nuke.createNode('LiveGroup', "file {%s}" % (path))
        liveGroup['reload_script'].execute()
    ...
    def _create_read_node(self, path, sg_publish_data):
        ...
        if ext.lower() == ".abc":
            nuke.createNode("ReadGeo2", "file {%s} tile_color %s" % (path, 4194238719))
            return
        elif ext.lower() == ".fbx":
            nuke.createNode("ReadGeo2", "file {%s} tile_color %s" % (path, 4194238719))
            return
        ...
        # read_node["updateUI"].setValue("checkMedia.checkMediaUI()")
    ...
    def _create_camera_node(self, path, sg_publish_data):
        """
        Create a camera node representing the publish.
        ...
        nuke.createNode("Camera2", "name {} read_from_file True file {} frame_rate {} tile_color {}".format(name, path, fps, 2822319615))

    def _create_audioread_node(self, path, sg_publish_data):
        """
        Create an audioread node representing the publish.
        ...
        audio_read_node = nuke.createNode("AudioRead", "file {%s}" % (path))
        audio_read_node.knob("tile_color").setValue(949013503)
        audio_read_node.knob("note_font_color").setValue(4294967295)
