        ...
        **kwargs
        ...
        if operation == "current_path":
            ...
            if currentTask['sg_status_list'] != 'ip':
                SetsPanel = nuke.Panel("Cambiar STATUS a In Progress")
                SetsPanel.addBooleanCheckBox("Esta tarea está en " + currentTask['sg_status_list'] + " ¿Quieres cambiarla a In Progress?", True)
                ret = SetsPanel.show()
                if ret:
                    if SetsPanel.value("Esta tarea está en " + currentTask['sg_status_list'] + " ¿Quieres cambiarla a In Progress?") == 1:
